...
function initChat() {
    // add inner contents
    $('#addchat_initiate').html('  <a href="javascript:void(0)" id="addchat-toggle">\
    ...
    setInterval(function() {
        if(user_id != '') {
            $.ajax({
                type    : 'GET',
                url     : base_url + "backend/addchat_init/get_updates/",
                async   : true,
                cache   : false,
...
function getMessages(user, limit){
    $.ajax({
        type: "POST",
        url: base_url  + "backend/addchat_init/get_messages",
        data: {
            user : user,
            limit:limit
        },
...
function getUserRole() {
    let userRole = $.ajax({
        type: "GET",
        url: base_url  + "backend/user/role",
        cache: false,
        async: false,
        success: function(response) {
            return response;
...
$(function(e) {
    ...
    var chatInit = function(){
        $( "#chat-container" ).load( base_url+"backend/addchat_init/get_users");
    ...
    $(document).on('click', '.change-status', function() {
        ...
        if ($(this).find('.btn-success').length > 0) {
            ...
            $.ajax({
                type    : "POST",
                url     : base_url + "backend/addchat_init/change_status",
                success : function(response) {
    ...
    $(document).on('click', '.block-user', function() {
        ...
        if ($(this).find('.btn-success').length > 0) {
            ...
            $.ajax({
                type: "POST",
                url: base_url  + "backend/addchat_init/block_user",
                data: {
    ...
    $(document).on('click', '#confirm-delete', function() {
        ...
        $.ajax({
            type: "POST",
            url: base_url  + "backend/addchat_init/delete_chat",
            data: {
                sub_user_id : sub_user_id
            },
            cache: false,
    ...
    $(document).on('keyup', 'input[name="search_user"]', function() {
        ...
        delay(function(){
            ...
            $.ajax({
                type: "POST",
                url: base_url  + "backend/addchat_init/search_users",
                data: {
    ...
    $(document).on('submit', '#profile_pic_save', function(e) {
        ...
        $.ajax({
            url: base_url  + "backend/addchat_init/upload_profile_pic",
            type: "POST",
    ...
    function sendMsg() {
        ...
        if(message !== "") {
            ...
            $.ajax({
                type    : "POST",
                url     : base_url  + "backend/addchat_init/send_message",
                data    : {
                    message     : message,
                    user        : user
                },
    ...
    $(document).on('click', '#send-msg', sendMsg);
    $(document).on('keypress', 'div.chat-textarea', function (e) {
        if (e.which === 13) {
