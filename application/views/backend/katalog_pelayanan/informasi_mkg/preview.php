    ...
    <div class="row">
      <div class="col-xs-12">
        <div class="box box-info">
          ...
          <div class="box-body">
            <table class="table table-striped">
              <tbody>
                <tr>
                  <td style="width: 400px;">No Permohonan</td>
		...
		<tr>
              ...
              <!-- Baris akhir penambahan tanggal permohonan, juml pembayaran, bukti pembayaran (Nurhayati Rahayu 25/07/2023) -->

                <tr>
                  <td>Status</td>
                  <td style="width: 10px">:</td>
                  <!--<td><?= status($detail['status'], $this->session->userdata('id_role')) ?></td> -->
                  <!-- Awal script yang diedit Nurhayati Rahayu 03 Mei 2023 -->
                  <?php if($detail['status'] == 0) { ?>
                      <td>Transaksi Masuk</td>
                  <?php } elseif($detail['status'] == 1) { ?>
                      <td>Menunggu Kelengkapan Data</td>
                  <?php } elseif($detail['status'] == 2) { ?>
                      <td>Proses Dihentikan</td>
                  <?php } elseif($detail['status'] == 3) { ?>
                      <td>Menunggu Bukti Pembayaran</td>
                  <?php } elseif($detail['status'] == 4) { ?>
                      <td>Verifikasi pembayaran</td>
                  <?php } elseif($detail['status'] == 5) { ?>
                      <td>Proses Dihentikan</td>
                  <?php } elseif($detail['status'] == 6) { ?>
                      <td>Konfirmasi Pengiriman</td>
                  <?php } elseif($detail['status'] == 7) { ?>
                      <td>Transaksi sukses (Pelanggan belum Mengisi IKM)</td>
                  <?php } elseif($detail['status'] == 8) { ?>
                      <td>Menunggu Ulang Bukti Pembayaran</td>
                  <?php } elseif($detail['status'] == 9) { ?>
                      <td>Verifikasi akhir</td>
                  <?php } elseif($detail['status'] == 10) { ?>
                      <td>Transaksi sukses (Pelanggan sudah mengisi IKM)</td>
                  <?php } ?>
                  <!-- Akhir script yang diedit Nurhayati Rahayu 03 Mei 2023 -->
                </tr>
              </tbody>
            </table>
          </div>
