 ...
 */
class Back_office extends MY_Controller {
    var $page_title = 'Back Office';
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'back_office_model', 'app_model');
...
"processing": true,
        ...
        $this->template->write_view('content', $this->folder.'/back_office/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
            'role' => strip_tags($this->input->post('back_office')),
            );
        ...
        $this->template->write_view('content', $this->folder.'/back_office/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_update = array(    'role' => strip_tags($this->input->post('back_office')),
            );
