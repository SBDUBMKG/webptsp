 ...
 */
class Pertanyaan_survey extends MY_Controller {
    var $page_title = 'Pertanyaan Survey';
    var $column_datatable = array('id_pertanyaan_survey', 'kategori_survey','pertanyaan_survey');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'pertanyaan_survey_model', 'app_model');
...
"processing": true,
        ...
        $this->template->write_view('content', $this->folder.'/pertanyaan_survey/datatable', $data, true);
    ...
    private function valid_form($act = 'add') {
        ...
        $config = array(
                    array(
                        'field' => 'id_kategori_survey',
                        'label' => 'Id Kategori Survey',
                        'rules' => 'integer'
                    ),
                
                    array(
                        'field' => 'pertanyaan_survey',
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
                'id_kategori_survey' => $this->input->post('id_kategori_survey'),
                'pertanyaan_survey' => strip_tags($this->input->post('pertanyaan_survey')),
                'pertanyaan_survey_en' => strip_tags($this->input->post('pertanyaan_survey_en')),
                );
        ...
        $this->template->write_view('content', $this->folder.'/pertanyaan_survey/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
