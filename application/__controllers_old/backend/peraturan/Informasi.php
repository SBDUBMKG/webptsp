...
class Informasi extends MY_Controller {
    var $page_title = 'Kelola Informasi';
    var $column_datatable = array('id_informasi', 'bidang','informasi','informasi_en','isi','isi_en','lampiran');
    var $folder         = 'backend/peraturan';
    var $module         = '';

    function __construct(){
        parent::__construct();
        $module = $this->folder.'/'.$this->router->fetch_class();
        $this->load->model($this->folder.'/'.'informasi_model', 'app_model');
...
"processing": true,
        ...
        $this->template->write_view('content', $this->folder.'/informasi/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
                    ...
                    'informasi' => strip_tags($this->input->post('informasi')),
                    'informasi_en' => strip_tags($this->input->post('informasi_en')),
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    $upload_conf = array(
                        'upload_path'       => 'upload/informasi/',
                        'allowed_types'     => 'pdf|doc|png|jpg|gif|docx',
                        'max_size'          => '30000',
        ...
        $this->template->write_view('content', $this->folder.'/informasi/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST) {
            ...
            if ( $simpan ) {
                $data_update = array(    
                    ...
                    'isi' => $this->input->post('isi'),
                    'isi_en' => $this->input->post('isi_en'),
                    'lampiran' => strip_tags($this->input->post('lampiran')),
                );
                $this->load->library('upload');
                $tmp_name = $_FILES['lampiran']['tmp_name'];
                if ( !empty($tmp_name) ) {
