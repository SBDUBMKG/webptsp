...
class Peraturan extends MY_Controller {
    var $page_title = 'Kelola Peraturan';
    var $column_datatable = array('id_peraturan', 'judul_peraturan','judul_peraturan_en','lampiran');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'peraturan_model', 'app_model');
...
"processing": true,
    ...
    private function valid_form($act = 'add') {
        ...
        $config = array(
                    array(
                        'field' => 'judul_peraturan',
                        'label' => 'Judul Peraturan',
                        'rules' => 'max_length[200]'
                    ),
                
                    array(
                        'field' => 'judul_peraturan_en',
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
                    'judul_peraturan' => strip_tags($this->input->post('judul_peraturan')),
                    'judul_peraturan_en' => strip_tags($this->input->post('judul_peraturan_en')),
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    $upload_conf = array(
                        'upload_path'       => realpath(PERATURAN),
    ...
    function edit($id = 0) {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_update = array(    'judul_peraturan' => strip_tags($this->input->post('judul_peraturan')),
                    'judul_peraturan_en' => strip_tags($this->input->post('judul_peraturan_en')),
                    'lampiran' => strip_tags($this->input->post('lampiran')),
                );
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    if ( !$this->upload->do_upload('lampiran') ) {
                        ...
                        $insert = $this->app_model->update_data($id,$data_update);
                        if ( $insert ) {
                            redirect(base_url().$module);
                        } else {
                            $errMsg = 'Data gagal disimpan';
