 ...
 */
class Jenis_paparan extends MY_Controller {
    var $page_title = 'Jenis Paparan';
    var $column_datatable = array('id_jenis_paparan', 'jenis_paparan');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'jenis_paparan_model', 'app_model');
    ...
    public function index()
    {
        ...
        $this->template->write_view('content', $this->folder.'/jenis_paparan/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
            'jenis_paparan' => strip_tags($this->input->post('jenis_paparan')),
            );

                $insert = $this->app_model->insert_data($data_insert);
                if ( $insert ) {					insert_log('Tambah jenis paparan');
        ...
        $this->template->write_view('content', $this->folder.'/jenis_paparan/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_update = array(    'jenis_paparan' => strip_tags($this->input->post('jenis_paparan')),
            );
                $update = $this->app_model->update_data($id, $data_update);
                if ( $update ) {					insert_log('Ubah jenis paparan');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
    ...
    function delete($id = '') {		
		insert_log('Hapus jenis paparan');
