 ...
 */
class Pengumuman_perizinan extends MY_Controller {
    var $page_title = 'Kelola Pengumuman Perizinan';
    var $column_datatable = array('id_pengumuman_perizinan', 'judul','judul_en','tanggal','lampiran','status');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'pengumuman_perizinan_model', 'app_model');
...
"processing": true,
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    $upload_conf = array(
                        'upload_path'       => './upload/pengumuman_perizinan/',
                        'allowed_types'     => 'pdf',
                        'max_size'          => '30000',
                    ...
                    if ( !$this->upload->do_upload('lampiran') ) {
                        ...
                        if ( $insert ) {                        							insert_log('Tambah pengumuman perizinan<br/><p><em>'.$data_insert['judul'].'</em></p>');
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    if ( !$this->upload->do_upload('lampiran') ) {
                        ...
                        if ( $update ) {							insert_log('Edit pengumuman perizinan<br/><p><em>'.$data_update['judul'].'</em></p>');
                            redirect(base_url().$module);
                        } else {
                            $errMsg = 'Data gagal disimpan';
                        }
    ...
    function delete($id = '') {		
		...
		if ( $detail )
			insert_log('Hapus pengumuman perizinan<br/><p><em>'.$detail['judul'].'</em></p>');
