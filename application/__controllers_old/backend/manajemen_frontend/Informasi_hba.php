 ...
 */
class Informasi_hba extends MY_Controller {
    var $page_title = 'Kelola Informasi HBA';
    var $column_datatable = array('id_informasi_hba', 'judul','judul_en','tanggal','lampiran');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'informasi_hba_model', 'app_model');
...
"processing": true,
    ...
    function add() {
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name_sampul) ) {
                    ...
                    $upload_conf_sampul = array(
                        'upload_path'       => './upload/informasi_hba/',
        ...
        $this->template->write_view('content', $this->folder.'/informasi_hba/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
