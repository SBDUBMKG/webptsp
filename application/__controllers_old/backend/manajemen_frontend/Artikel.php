...
class Artikel extends MY_Controller {
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'artikel_model', 'app_model');
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    if ( !$this->upload->do_upload('thumbnail') ) {
                        ...
                        $data_insert['gambar'] = $_SESSION['artikel'][0];
                        $insert = $this->app_model->insert_data($data_insert);
                        if ( $insert ) {							
							insert_log('Tambah artikel<br/><p><em>'.$data_insert['judul'].'</em></p>');
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    if ( !$this->upload->do_upload('thumbnail') ) {
                        ...
                        if ( $update ) {							
							insert_log('Tambah artikel<br/><p><em>'.$data_update['judul'].'</em></p>');
                            redirect(base_url().$module);
                        } else {
                            $errMsg = 'Data gagal disimpan';
                        }
    ...
    function delete($id = '') {		
		...
		if ( $detail )
			insert_log('Hapus artikel<br/><p><em>'.$detail['judul'].'</em></p>');
        $this->app_model->delete_data($id);
        redirect(base_url().$this->module);
    }

    function save_image(){
        if(move_uploaded_file($_FILES['file']['tmp_name'],'upload/artikel/'.$_FILES['file']['name'])){
            echo base_url().'upload/artikel/'.$_FILES['file']['name'];
            if(!isset($_SESSION['artikel'])){
                $_SESSION['artikel'] = array();
            }
            array_push($_SESSION['artikel'] , $_FILES['file']['name']);
        }
    }
}
