 ...
 */
class Pengumuman_cnc extends MY_Controller {
    var $page_title = 'Kelola Pengumuman CNC';
    var $column_datatable = array('id_pengumuman_cnc', 'judul','judul_en','tanggal','lampiran');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'pengumuman_cnc_model', 'app_model');
...
"processing": true,
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                $lampiran_text = $this->input->post("lampiran_text");
                $data_insert = array(
                    ...
                    'jenis_lampiran' => strip_tags($jenis_lampiran),
                );
                ...
                if ($jenis_lampiran==1) {
                    ...
                    if ( !empty($tmp_name) ) {
                        ...
                        $upload_conf = array(
                            'upload_path'       => './upload/pengumuman_cnc/',
                            'allowed_types'     => 'pdf',
                            'max_size'          => '30000',
                ...
                else if($jenis_lampiran==2) {
                    //TEXT
                    $data_insert['lampiran'] = $lampiran_text;
                }


                $insert = $this->app_model->insert_data($data_insert);                  
                if ( $insert ) {
                    insert_log('Tambah pengumuman cnc<br/><p><em>'.$data_insert['judul'].'</em></p>');
        ...
        $this->template->write_view('content', $this->module.'/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    if ( !$this->upload->do_upload('lampiran') ) {
                        ...
                        if ( $update ) {							insert_log('Edit pengumuman cnc<br/><p><em>'.$data_update['judul'].'</em></p>');
                            redirect(base_url().$module);
                        } else {
                            $errMsg = 'Data gagal disimpan';
                        }
    ...
    function delete($id = '') {		
		...
		if ( $detail )
			insert_log('Hapus pengumuman cnc<br/><p><em>'.$detail['judul'].'</em></p>');
