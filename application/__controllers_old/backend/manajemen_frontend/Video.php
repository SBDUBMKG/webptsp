...
class Video extends MY_Controller {
    var $page_title = 'Kelola Video';
    var $column_datatable = array('id_video', 'judul','embed','upload');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'video_model', 'app_model');
...
"processing": true,
        ...
        $this->template->write_view('content', $this->folder.'/video/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( !empty($tmp_name) ) {
                $extension_expl = explode('.', $_FILES['uploadvideo']['name']);
                $extension = $extension_expl[count($extension_expl)-1];
                $file_name = date("YmdHis");

                $upload_conf = array(
                    'upload_path'       => './upload/video/',
                    'allowed_types'     => 'mkv|mp4|mpeg|flv|mov|swf|3gp',
                    'max_size'          => '30000',
            ...
            if ( $simpan ) {
                ...
                if ( $insert ) {					insert_log('Tambah video<br/><p><em>'.$data_insert['judul'].'</em></p>');
        ...
        $this->template->write_view('content', $this->folder.'/video/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( $update ) {					insert_log('Edit video<br/><p><em>'.$data_update['judul'].'</em></p>');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
    ...
    function delete($id = '') {
		...
		if ( $detail )
			insert_log('Hapus video<br/><p><em>'.$detail['judul'].'</em></p>');
        $file = 'upload/video/'.$detail['upload'];
        unlink($file);
