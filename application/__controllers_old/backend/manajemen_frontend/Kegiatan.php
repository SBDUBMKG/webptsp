...
class Kegiatan extends MY_Controller {
    ...
    var $column_datatable = array('id_kegiatan', 'judul','judul_en','tanggal','keterangan','keterangan_en');
...
"processing": true,
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( $insert ) {									insert_log('Tambah kegiatan<br/><p><em>'.$data_insert['judul'].'</em></p>');					
                    if(!empty($_FILES['userFiles']['name'])){
                        ...
                        if(!empty($uploadData)){
                            $insert_foto = $this->global_model->insert_foto('r_galeri_foto', $uploadData);
                        }
                    }
    ...
    function edit($id = 0) {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( $update ) {					insert_log('Edit kegiatan<br/><p><em>'.$data_update['judul'].'</em></p>');
                    if(!empty($_FILES['userFiles']['name'])){
                        $filesCount = count($_FILES['userFiles']['name']);
                        for($i = 0; $i < $filesCount; $i++){
                            $_FILES['userFile']['name'] = $_FILES['userFiles']['name'][$i];
    ...
    function delete_foto($id,$id_kegiatan){
        $url = $_SESSION['url'];
        $this->load->model('global_model');
        $delete = $this->global_model->delete_data('r_galeri_foto', 'id_fotokegiatan', $id);
        if($delete){
            unlink($_SESSION['url']);
            unset($_SESSION['url']);
    ...
    function default_foto($id_fotokegiatan,$id_kegiatan){
        $this->load->model('global_model');
        $reset_default = $this->global_model->update_data('r_galeri_foto', 'id_kegiatan', $id_kegiatan, array('is_default' => 0));
        if($reset_default){
            $this->global_model->update_data('r_galeri_foto', 'id_fotokegiatan', $id_fotokegiatan, array('is_default' => 1));
            redirect(base_url().$this->module.'/edit/'.$id_kegiatan);
        }
    }
}
