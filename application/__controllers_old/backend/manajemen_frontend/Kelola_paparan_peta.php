 ...
 */
class Kelola_paparan_peta extends MY_Controller {
    var $page_title = 'Kelola Paparan Peta';
    var $column_datatable = array('id_paparan', 'judul','jenis_paparan','lampiran');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'kelola_paparan_peta_model', 'app_model');
...
"processing": true,
        ...
        $this->template->write_view('content', $this->folder.'/kelola_paparan_peta/datatable', $data, true);
    ...
    private function valid_form($act = 'add') {
        ...
        $config = array(
                    ...
                    array(
                        'field' => 'id_jenis_paparan',
                        'label' => 'Id Jenis Paparan',
                        'rules' => 'integer'
                    ),
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    ...
                    $upload_conf = array(
                        'upload_path'       => './upload/paparan_peta/',
                        'allowed_types'     => 'pdf|doc|png|jpg|gif',
                        'max_size'          => '30000',
                ...
                if ( !empty($tmp_name) ) {
                    $data_insert = array(
                        ...
                        'id_jenis_paparan' => $this->input->post('id_jenis_paparan'),
                        'lampiran'         => $upload_conf['file_name'],
                    );                    
                }
                else{
                    $data_insert = array(
                ...
                if ( $insert ) {					insert_log('Tambah paparan peta<br/><p><em>'.$data_insert['judul'].'</em></p>');
        ...
        $this->template->write_view('content', $this->folder.'/kelola_paparan_peta/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( !empty($tmp_name) ) {
                    $this->load->model('global_model');
                    $paparan = $this->global_model->get_by_id('tbl_paparan', 'id_paparan', $id);
                    $file = 'upload/paparan_peta/'.$paparan->lampiran;
                    unlink($file);
                ...
                if ( $update ) {					insert_log('Edit paparan peta<br/><p><em>'.$data_update['judul'].'</em></p>');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
    ...
    function delete($id = '') {		
		...
		if ( $detail )
			insert_log('Hapus paparan peta<br/><p><em>'.$detail['judul'].'</em></p>');
        $this->load->model('global_model');
        $paparan = $this->global_model->get_by_id('tbl_paparan', 'id_paparan', $id);
        $file = 'upload/paparan_peta/'.$paparan->lampiran;
        unlink($file);		
