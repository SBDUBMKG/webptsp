...
class Menu extends MY_Controller {
    var $page_title = 'Kelola Menu';
    var $column_datatable = array('id_menu', 'kategori_menu','menu','cname','uri','urutan');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'menu_model', 'app_model');
    ...
    public function index()
    {
        ...
        $this->template->write_view('content', $this->folder.'/menu/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            'id_kategori_menu' => $this->input->post('id_kategori_menu'),
                ...
                if ( $insert ) {					insert_log('Tambah menu <em>'.$data_insert['menu'].'</em>');
        ...
        $this->template->write_view('content', $this->folder.'/menu/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_update = array(    'id_kategori_menu' => $this->input->post('id_kategori_menu'),
                'menu' => strip_tags($this->input->post('menu')),
                'cname' => strip_tags($this->input->post('cname')),
                'uri' => strip_tags($this->input->post('uri')),
                'urutan' => $this->input->post('urutan'),
                ...
                if ( $update ) {					insert_log('Edit menu <em>'.$data_update['menu'].'</em>');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
    ...
    function delete($id = '') {		$detail = $this->app_model->get_by_id($id);		if ( $detail ) 			insert_log('Hapus menu <em>'.$detail['menu'].'</em>');
