...
class Bidang extends MY_Controller {
    var $page_title = 'Kelola Bidang';
    var $column_datatable = array('id_bidang', 'bidang');
    var $folder         = 'backend/layanan';
    var $module         = '';

    function __construct(){
        parent::__construct();
        $module = $this->folder.'/'.$this->router->fetch_class();
        $this->load->model($this->folder.'/'.'bidang_model', 'app_model');
    ...
    public function index()
    {
        ...
        $this->template->write_view('content', $this->folder.'/bidang/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
            'bidang' => strip_tags($this->input->post('bidang')),
            );
        ...
        $this->template->write_view('content', $this->folder.'/bidang/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_update = array(    'bidang' => strip_tags($this->input->post('bidang')),
            );
