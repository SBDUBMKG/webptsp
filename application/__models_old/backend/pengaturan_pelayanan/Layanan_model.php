...
class Layanan_model extends CI_Model {
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_layanan, layanan, layanan_en, is_produk, icon');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $icon = !empty($aRow['icon']) ? '<img src="'.base_url().'upload/icon/'.$aRow['icon'].'" width="30px"/>' : '<img src="'.base_url().'/resources/frontend/images/katalog.png" width="30px"/>';
            $row[] = $icon;
            $row[] = '';
