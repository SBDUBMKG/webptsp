...
class Laporan_keuangan_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        $where = "1=1 AND tbl_permohonan.status IN(7,10)";
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_detail_permohonan, no_permohonan, jumlah, harga, tbl_permohonan.status');
        $this->db->from($this->table);
        $this->db->join('tbl_permohonan', 'tbl_permohonan.id_permohonan = '.$this->table.'.id_permohonan', 'left');
