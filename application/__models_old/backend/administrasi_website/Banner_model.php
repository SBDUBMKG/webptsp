...
class Banner_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_banner, tema, banner, banner_en, is_active');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $aktif = $aRow['is_active'] == 1 ? '<small class="label label-success">Ya</small>' : '<small class="label label-danger">Tidak</small>';

            $row[] = $aRow['tema'];
            $row[] = '<a href="'.base_url().BANNER.$aRow['banner'].'" target="blank"><img width="200px" src="'.base_url().BANNER.$aRow['banner'].'"/></a>';
            $row[] = '<a href="'.base_url().BANNER.$aRow['banner_en'].'" target="blank"><img width="200px" src="'.base_url().BANNER.$aRow['banner_en'].'"/></a>';
            $row[] = $aktif;
