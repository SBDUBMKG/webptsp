        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                {
                    if ( $aColumns[$i] == 'program_kerja' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'program_kerja_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id, program_kerja, program_kerja_en');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['program_kerja'];
            $row[] = $aRow['program_kerja_en'];                
            $row[] = '';
