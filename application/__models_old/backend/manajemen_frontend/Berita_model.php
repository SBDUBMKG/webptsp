...
class Berita_model extends CI_Model
{
    ...
    var $m_jenis_konten = 'm_jenis_konten';
    var $pk_name    = 'id';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id, judul, judul_en, tanggal_berita, isi_en, isi, thumbnail,jenis_konten,is_publish');
        $this->db->from($this->table.' A');
        $this->db->join($this->m_jenis_konten.' B','A.id_jenis_konten=B.id_jenis_konten','LEFT');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['jenis_konten'];
            $row[] = $aRow['judul'];
            $row[] = $aRow['judul_en'];
            $row[] = empty($aRow['tanggal_berita']) || $aRow['tanggal_berita'] == '0000-00-00' ? NULL : format_datetime($aRow['tanggal_berita']) ;
            $row[] = empty($aRow['is_publish']) || $aRow['is_publish'] == '0' ? 'Tidak' : 'Ya';
