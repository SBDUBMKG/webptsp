...
{
    var $table      = 'tbl_pengumuman';
    var $pk_name    = 'id_pengumuman';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_pengumuman, judul, judul_en, isi, isi_en, file, created_by, created_date, is_publish, expired_date');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['judul'];
                $row[] = $aRow['judul_en'];
                $row[] = substr($aRow['isi'], 0,100);
                $row[] = substr($aRow['isi_en'], 0,100);
                // $row[] = $aRow['file'];
                $row[] = $aRow['created_by'];
                $row[] = format_datetime($aRow['created_date']);
                $row[] = $aRow['is_publish'] == 1 ? '<small class="label bg-green">Ya</small>' : '<small class="label bg-red">Tidak</small>';
                $row[] = format_datetime($aRow['expired_date']);
