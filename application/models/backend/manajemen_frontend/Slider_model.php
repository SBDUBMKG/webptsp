...
class Slider_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        if (isset($order) < 1 ) {
            $this->db->order_by('is_active desc, (urutan * -1) desc ');
        }
        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                if( $columns[$i]['searchable'] == 'true')
                {
                    ...
                    else if ( $aColumns[$i] == 'slider' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'slider_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_slider, link, slider, slider_en, urutan, is_active');
        ...
        $this->db->select('id_slider, link, slider, slider_en, urutan, is_active');
        $this->db->from($this->table);
        $this->db->order_by('is_active, urutan');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $aktif = $aRow['is_active'] == 1 ? '<small class="label label-success">Publish</small>' : '<small class="label label-danger">Tidak</small>';

            $row[] = $aRow['link'];
            $row[] = '<a href="'.base_url().'upload/slider/'.$aRow['slider'].'" target="blank"><img width="200px" src="'.base_url().'upload/slider/'.$aRow['slider'].'"/></a>';
            $row[] = '<a href="'.base_url().'upload/slider/'.$aRow['slider_en'].'" target="blank"><img width="200px" src="'.base_url().'upload/slider/'.$aRow['slider_en'].'"/></a>';
            $row[] = '<center>'.$aRow['urutan'].'</center>';
            $row[] = $aktif;
