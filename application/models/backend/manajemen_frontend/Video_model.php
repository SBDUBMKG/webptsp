...
class Video_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_video, judul, embed, upload');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            if(empty($aRow['embed'])){
                ...
                $embed = str_replace('width="560"', 'width="300px"', $aRow['embed']);
                $embed = str_replace('height="315"', 'height="200px"', $embed);
                $row[] = $embed;
                // $row[] = $aRow['embed'];
            }

            if(empty($aRow['upload'])){
                $row[] = ' - ';
            }
            else{
                // $row[] = '<a href="'.base_url().'upload/video/'.$aRow['upload'].'" target="blank">'.$aRow['upload'].'</a>';
                $row[] = '<video width="300" controls>
                        <source src="'.base_url().'upload/video/'.$aRow['upload'].'">
                        </video>';
            }
