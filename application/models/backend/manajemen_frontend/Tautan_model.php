...
{
    var $table      = 'tbl_tautan';
    var $pk_name    = 'id_tautan';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                if( $columns[$i]['searchable'] == 'true')
                {
                    ...
                    else if ( $aColumns[$i] == 'link' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'gambar' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_tautan, judul, jenis_tautan, link, gambar,urutan');
        $this->db->from($this->table);
        $this->db->join('m_jenis_tautan', 'tbl_tautan.id_jenis_tautan = m_jenis_tautan.id_jenis_tautan');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['judul'];
            $row[] = $aRow['jenis_tautan'];
            $row[] = $aRow['link'];
            $row[] = '<img width="200px" src="'.base_url().'upload/tautan/'.$aRow['gambar'].'"/>';
            $row[] = $aRow['urutan'];
