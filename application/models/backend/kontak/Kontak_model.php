...
class Kontak_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                if( $columns[$i]['searchable'] == 'true')
                {
                    if ( $aColumns[$i] == 'nama' )
                        ...
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'pesan' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'jawaban' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_kontak, nama, email, pesan, jawaban, tgl_kirim, is_publish');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['jawaban'];
            $row[] = $aRow['tgl_kirim'];
            $publish = $aRow['is_publish'] == 1 ? '<small class="label label-success">Ya</small>' : '<small class="label label-danger">Tidak</small>';
            $row[] = $publish;
