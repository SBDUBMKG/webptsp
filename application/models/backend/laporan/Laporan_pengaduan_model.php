...
class Laporan_pengaduan_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch,$tahun,$bulan){
        ...
        */
        $where = "1=1";
        $where .= !empty($tahun) ? ' AND YEAR(waktu_pengaduan)='.$tahun : NULL;
        $where .= !empty($bulan) && $bulan !== 'all' ? ' AND MONTH(waktu_pengaduan)='.$bulan : NULL;
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_pengaduan, waktu_pengaduan, nama, email, pengaduan, response');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = empty($aRow['waktu_pengaduan']) || $aRow['waktu_pengaduan'] == '0000-00-00' ? NULL : short_datetime($aRow['waktu_pengaduan']) ;
            $row[] = $aRow['nama'];
            $row[] = $aRow['email'];
            $row[] = $aRow['pengaduan'];
            $row[] = $aRow['response'];
