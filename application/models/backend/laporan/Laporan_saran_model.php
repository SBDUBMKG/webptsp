...
class Laporan_saran_model extends CI_Model
{
    var $table      = 'tbl_saran';
    var $pk_name    = 'id_saran';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch,$tahun,$bulan){
        ...
        */
        $where = "1=1";
        $where .= !empty($tahun) ? ' AND YEAR(waktu_saran)='.$tahun : NULL;
        $where .= !empty($bulan) && $bulan !== 'all' ? ' AND MONTH(waktu_saran)='.$bulan : NULL;
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                if( $columns[$i]['searchable'] == 'true')
                {
                    if ( $aColumns[$i] == 'nama' )
                        ...
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'saran' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'response' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_saran, waktu_saran, nama, email, saran, response');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = empty($aRow['waktu_saran']) || $aRow['waktu_saran'] == '0000-00-00' ? NULL : short_datetime($aRow['waktu_saran']) ;
            $row[] = $aRow['nama'];
            $row[] = $aRow['email'];
            $row[] = $aRow['saran'];
            $row[] = $aRow['response'];
