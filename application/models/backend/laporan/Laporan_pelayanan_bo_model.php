...
class Laporan_pelayanan_bo_model extends CI_Model
{
    var $view      = 'v_lap_pelayanan_bo';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch,$jenis_layanan,$tahun,$bulan){        
        ...
        */
	    ...
	    // $where .= !empty($status_transaksi) ? ' AND status_transaksi LIKE "%'.$status_transaksi.'%"' : NULL;
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                if( $columns[$i]['searchable'] == 'true')
                {
                    ...
                    //     $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                        ...
                        $where .= $aColumns[$i]." IN {'%".$this->db->escape_like_str($sSearch['value'])."%'} OR ";
        ...
        $this->db->where($where);
        // $this->db->select('id_detail_permohonan, tanggal_permohonan, no_permohonan, nama, perusahaan, email, no_telepon, id_jenis_layanan,jenis_layanan, layanan, tarif_pnbp, jumlah, total_tarif_pnbp, status_data, status_transaksi,petugaskonfirmasi,petugasverifikasi');
        $this->db->select('id_detail_permohonan, tanggal_permohonan, no_permohonan, tanggal_verifikasibendahara, nama, perusahaan, email, no_telepon, id_jenis_layanan,jenis_layanan, layanan, status_transaksi');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = empty($aRow['tanggal_verifikasibendahara']) || $aRow['tanggal_verifikasibendahara'] == '0000-00-00' ? NULL : $aRow['tanggal_verifikasibendahara'] ;
            $row[] = $aRow['nama'];
            $row[] = $aRow['perusahaan'];
            //Baris awal penambahan kolom email. Perbaikan oleh Nurhayati Rahayu 17/10/2022
            $row[] = $aRow['email'];
