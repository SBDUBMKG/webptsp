...
class Kategori_menu_frontend_model extends CI_Model
{
    var $table      = 'tbl_kategori_menu_frontend';
    var $pk_name    = 'id_kategori_menu';
    var $module     = '';
        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                {
                    if ( $aColumns[$i] == 'kategori_menu' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'kategori_menu_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_kategori_menu, kategori_menu, kategori_menu_en, uri, icon, urutan');
