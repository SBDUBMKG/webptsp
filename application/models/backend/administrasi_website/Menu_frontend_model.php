    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            // Button edit dan delete refer to row id. Perbaikan oleh Aminulloh Zaqi D (20 Mei 2024)
            if ( $this->is_write ) {
                $row[] .= "<center>
				<a href='".base_url().strtolower($this->module).'/edit/'.$aRow['id']."'><span class='btn btn-xs btn-success'><i class='fa fa-pencil'></i> Edit</span></a>
                <a href='".base_url().strtolower($this->module).'/delete/'.$aRow['id']."' onclick='return confirm(\"Apa anda yakin menghapus data ini?\")'><span class='btn btn-xs btn-danger'><i class='fa fa-trash'></i> Hapus</span></a>
				</center>";
            }
            //Baris awal penggantian tombol navigasi. Perbaikan oleh Nurhayati Rahayu (17 Mei 2024)
