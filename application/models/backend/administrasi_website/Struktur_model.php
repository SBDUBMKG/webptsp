...
class Struktur_model extends CI_Model
{
    ...
    var $pk_name    = 'id_struktur_organisasi';
    var $module     = '';
        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                {
                    if ( $aColumns[$i] == 'profil' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'profil_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'struktur_organisasi' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'struktur_organisasi_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_struktur_organisasi, profil, profil_en, struktur_organisasi, struktur_organisasi_en');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['profil'];
            $row[] = $aRow['profil_en'];
            $row[] = '<img src="'.base_url().'resources/frontend/images/'.$aRow['struktur_organisasi'].'" width="150px"/>';
            $row[] = '<img src="'.base_url().'resources/frontend/images/'.$aRow['struktur_organisasi_en'].'" width="150px"/>';
