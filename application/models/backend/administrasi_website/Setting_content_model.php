<?php
//file: application\models\backend\administrasi_website\Setting_content_model.php
defined('BASEPATH') OR exit('No direct script access allowed');
class Setting_content_model extends CI_Model
{
    var $table      = 'tbl_setting_content';
    var $pk_name    = 'id_task';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_task, variable_task, value_task');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $var_task = $aRow['variable_task'];
            $row[] = $var_task;
            $value_task =  $aRow['value_task'];
            if($var_task =='smtp_pass'){
                $value_task = sha1(md5($value_task));
            }
            $row[] = $value_task;
