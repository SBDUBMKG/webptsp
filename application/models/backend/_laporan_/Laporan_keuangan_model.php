...
class Laporan_keuangan_model extends CI_Model
{
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        // Awal script yang ditambahkan Rahmat, 17 Juni 2020 dan Nurhayati Rahayu, 24 Juni 2020
        $this->db->select('A.id_detail_permohonan, B.no_permohonan, C.nama, E.perusahaan, D.layanan, A.jumlah, A.harga, B.bukti, B.status');
        $this->db->from($this->table.' A');
		...
		// Akhir script yang ditambahkan Rahmat, 17 Juni 2020  dan Nurhayati Rahayu, 24 Juni 2020      
        ...
        */
	    ...
	    // Awal script yang ditambahkan Nurhayati Rahayu, 24 Juni 2020
            ...
            $row[]  = empty($aRow['bukti']) ? NULL : '<a class="btn btn-xs btn-success" href="'.base_url().'upload/bukti/'.$aRow['bukti'].'">'.$icon_file.' Bukti</a>  ';
	    // Akhir script yang ditambahkan Nurhayati Rahayu, 24 Juni 2020
	
            $row[] = status_lengkap($aRow['status']);
