...
{
    var $table      = 'tbl_peraturan';
    var $pk_name    = 'id_peraturan';
    var $module     = '';
        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                {
                    if ( $aColumns[$i] == 'judul_peraturan' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'judul_peraturan_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_peraturan, judul_peraturan, judul_peraturan_en, lampiran');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['judul_peraturan'];
            $row[] = $aRow['judul_peraturan_en'];
            $row[] = '<a href="'.base_url().'upload/peraturan/'.$aRow['lampiran'].'"><i class="fa fa-search"></i> Lihat</a>';
