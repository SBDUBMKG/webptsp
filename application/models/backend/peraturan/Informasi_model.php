...
{
    var $table      = 'tbl_informasi';
    var $table_bidang      = 'tbl_bidang';
    var $pk_name    = 'id_informasi';
    var $module     = '';
        ...
        */
        ...
        if(isset($sSearch) && !empty($sSearch['value']))
        {
            ...
            for($i=0; $i<count($aColumns); $i++)
            {
                ...
                {
                    if ( $aColumns[$i] == 'informasi' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'informasi_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'isi' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
                    else if ( $aColumns[$i] == 'isi_en' )
                        $where .= $aColumns[$i]." LIKE '%".$this->db->escape_like_str($sSearch['value'])."%' OR ";
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_informasi, bidang, informasi, informasi_en, isi, isi_en, lampiran');
        $this->db->from($this->table.' A');
        $this->db->join($this->table_bidang.' B','A.id_bidang=B.id_bidang','left');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $nomor.'<input type="hidden" class="selectedrow" value="'.$aRow[$this->pk_name].'">';
                ...
                $row[] = $aRow['bidang'];
                $row[] = $aRow['informasi'];
                $row[] = $aRow['informasi_en'];
                $row[] = !empty($aRow['isi']) ?  substr($aRow['isi'],0,100) : NULL;
                $row[] = !empty($aRow['isi_en']) ? substr($aRow['isi_en'],0,100) : NULL;
                $row[] = !empty($aRow['lampiran']) ? '<a href="'.base_url().'upload/informasi/'.$aRow['lampiran'].'"> <i class="fa fa-download"></i> Lampiran </a>' : NULL;
