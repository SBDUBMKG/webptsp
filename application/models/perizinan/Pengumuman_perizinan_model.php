...
class Pengumuman_perizinan_model extends CI_Model
{
    var $table      = 'tbl_pengumuman_perizinan';
    var $pk_name    = 'id_pengumuman_perizinan';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch,$status){
        ...
        */
        $where = "1=1";
        $where .= $status == 1 ? ' AND status=1' : ' AND status=0';
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id_pengumuman_perizinan, judul, tanggal, lampiran');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['judul'];
            $row[] = $aRow['tanggal'];
            $row[] = '<a href="'.base_url().'upload/pengumuman_perizinan/'.$aRow['lampiran'].'" target="_blank"><i class="fa fa-download"></i> Lihat File</a>';
