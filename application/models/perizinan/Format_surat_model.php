...
class Format_surat_model extends CI_Model
{
    var $table      = 'tbl_format_surat';
    var $pk_name    = 'id';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch){
        ...
        */
        ...
        $this->db->where($where);
        // Select Data
        $this->db->select('id, judul, judul_en, keterangan, keterangan_en, lampiran, hit');
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = '<b>'.$aRow['judul'.$bahasa].'</b><br>'.$aRow['keterangan'.$bahasa];
            $row[] = '<a class="update_hit" href="'.base_url().'upload/format_surat/'.$aRow['lampiran'].'" target="_blank"><i class="fa fa-download"></i> Lihat File</a>';
            $row[] = !empty($aRow['hit']) ? $aRow['hit'] : 0;
            $row[] = '';
