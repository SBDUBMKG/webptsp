<?php
//file: application\controllers\Saran.php
...
class Saran extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('frontend/saran_model', 'app_model');
        $this->module = $module;
    }
    ...
    public function index() {
        ...
        $total_saran = $this->app_model->get_total_saran();
        $saran = $this->app_model->get_list_saran();
        $response = $this->app_model->get_list_response();
        $data['total_saran'] = $total_saran;
        $data['list_saran'] = $saran;
        ...
        if($_POST) {
            ...
            if($simpan){
                ...
                $lang = $this->session->userdata('bahasa');
                    $msg_berhasil = 'Saran anda berhasil dikirim.. terima kasih';
                    $msg_gagal = 'Saran gagal dikirim!';
                    if($lang == '_en'){
                ...
                if ( $insert ) {
                    //kirim email ke admin PTSP
                        $admin_ptsp = $this->global_model->get_by_id('tbl_admin','id_role',3);
                        $to = $admin_ptsp->email;
                        $subject = '[Notif-PTSP] - Saran Baru masuk';
        ...
        $this->template->write_view('content', 'v_saran', $data, true);
