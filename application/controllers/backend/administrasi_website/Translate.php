...
class Translate extends MY_Controller {
    var $page_title = 'Translate Template';
    var $column_datatable = array('id', 'nama','translate','translate_en');
    //Penambahan variable pencarian tanpa menggunakan tabel id. perubahan oleh : Nurhayati Rahayu (15/11/2019)
    var $column_datatable_search = array('nama','translate','translate_en');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'translate_model', 'app_model');
    ...
    public function index()
    {
        ...
        $this->template->write_view('content', $this->folder.'/translate/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            'nama' => strip_tags($this->input->post('nama')),
                ...
                'translate_en' => $this->input->post('translate_en'),
            );

                $insert = $this->app_model->insert_data($data_insert);
                if ( $insert ) {					insert_log('Tambah terjemahan');
        ...
        $this->template->write_view('content', $this->folder.'/translate/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( $update ) {					insert_log('Edit terjemahan');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
    ...
    function delete($id = '') {				insert_log('Hapus terjemahan');
