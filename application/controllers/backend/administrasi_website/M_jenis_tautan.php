...
class M_jenis_tautan extends MY_Controller {
    var $page_title = 'Master Jenis Tautan';
    //Penambahan variable pencarian tanpa menggunakan tabel id. perubahan oleh : Nurhayati Rahayu (15/11/2019)
    var $column_datatable_search = array('jenis_tautan');
    // baris terakhir perubahan
    var $column_datatable = array('id_jenis_tautan', 'jenis_tautan');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'m_jenis_tautan_model', 'app_model');
    ...
    public function index()
    {
        ...
        $this->template->write_view('content', $this->folder.'/m_jenis_tautan/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
            'jenis_tautan' => strip_tags($this->input->post('jenis_tautan')),
            );

                $insert = $this->app_model->insert_data($data_insert);
                if ( $insert ) {					insert_log('Tambah jenis tautan');
        ...
        $this->template->write_view('content', $this->folder.'/m_jenis_tautan/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_update = array(    'jenis_tautan' => strip_tags($this->input->post('jenis_tautan')),
            );
                $update = $this->app_model->update_data($id, $data_update);
                if ( $update ) {					insert_log('Ubah jenis tautan');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
