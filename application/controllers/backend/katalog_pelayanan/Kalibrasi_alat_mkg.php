...
class Kalibrasi_alat_mkg extends MY_Controller {
  public $page_title          = 'Jasa Kalibrasi Alat Meteorologi, Klimatologi dan Geofisika';
  ...
  public function __construct()
  {
    ...
    $this->load->model($this->folder . '/' . 'Kalibrasi_alat_mkg_model', 'app_model');
  ...
  public function index() 
  {
      ...
      $(function () {
        var oTable = $("#datatable").DataTable({
          ...
          buttons: [ ' . ($this->is_write ? '
              ...
              //Menghapus tombol Hapus Data. Perubahan oleh Nurhayati Rahayu (05042021)
			  //{ ' . ($id_role != 7 ? '
    ...
    $this->template->write_view('content', $this->folder . '/kalibrasi_alat_mkg/datatable', $data, true);
  ...
  public function add()
  {
    ...
    $this->template->write_view('content', $this->folder . '/kalibrasi_alat_mkg/form', $data, true);
    $this->template->render();
  }
  ...
  public function proses($id = 0)
  {
    ...
    if ($query->row() !== null) {
      foreach ($query->result() as $k => $v) {
        $data['layanan'] = $this->db->get_where('tbl_detail_permohonan',
          [
            'id_permohonan' => $v->id_permohonan,
  ...
  public function detail($id = 0)
  {
    ...
    $data['errMsg']     = $errMsg;
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/kalibrasi_alat_mkg/preview', $data, true);
    $this->template->render();
  }
  ...
  public function verifikasi_akhir($id = '')
  {
    ...
    if ($_POST) {
      ...
      if (empty($pengantar)) {
        $config_2['upload_path']      = './upload/pangantar/';
        $config_2['encrypt_name']     = true;
        $config_2['allowed_types']    = 'gif|jpg|jpeg|png|pdf|doc|docx|xls|xlsx|ppt|pptx|zip|rar';
        $config_2['file_ext_tolower'] = true;
        $this->load->library('upload', $config_2);
