...
class Informasi_mkg extends MY_Controller {
  public $page_title          = 'Pemesanan Produk';

  //Baris awal menambahkan kolom alamat. Perbaikan oleh Nurhayati Rahayu (15/03/2024)
  //public $column_datatable    = ['B.id_permohonan', 'B.no_permohonan', 'B.tanggal_permohonan', 'B.pengantar', 'A.nama', 'A.no_hp', 'B.status', 'B.tanggal_verifikasibendahara'];
  //public $column_datatable2   = ['B.id_permohonan', 'B.no_permohonan', 'B.tanggal_permohonan', 'B.pengantar', 'A.nama', 'A.no_hp', 'C.due_date', 'B.status', 'B.tanggal_verifikasibendahara'];
  public $column_datatable    = ['B.id_permohonan', 'B.no_permohonan', 'B.tanggal_permohonan', 'B.pengantar', 'A.nama', 'C.perusahaan', 'A.alamat', 'A.no_hp', 'B.status', 'B.tanggal_verifikasibendahara'];
  public $column_datatable2   = ['B.id_permohonan', 'B.no_permohonan', 'B.tanggal_permohonan', 'B.pengantar', 'A.nama', 'A.alamat', 'A.no_hp', 'C.due_date', 'B.status', 'B.tanggal_verifikasibendahara'];
  //Baris akhir menambahkan kolom alamat. Perbaikan oleh Nurhayati Rahayu (15/03/2024)

  public $folder              = 'backend/katalog_pelayanan';
  public $module              = '';
  ...
  public function index()
  {
    ...
    if($id_role > 8) {
      ...
      $(function () {
        $("#datatable").DataTable({
                    ...
                    "responsive": true,
                    "order": [ 0, "desc" ],
                    "ajax" : "'.base_url().$this->module.'/getDataTableBO"
                });
      });
      ';
    } else {
    ...
    //Baris akhir penggantian tombol navigasi. Perbaikan oleh Nurhayati Rahayu (17 Mei 2024)
    ...
    $this->template->write_view('content', $this->folder . '/informasi_mkg/datatable', $data, true);
  ...
  public function getDataTableBO()
  {
    $iDisplayStart  = $this->input->get_post('start', true);
    $iDisplayLength = $this->input->get_post('length', true);
    $order          = $this->input->get_post('order', true);
    ...
    $view = $this->app_model->show_datatable_bo($this->column_datatable2, $iDisplayStart, $iDisplayLength, $order, $sSearch);
  ...
  public function add()
  {
    ...
    $this->template->write_view('content', $this->folder . '/informasi_mkg/form', $data, true);
    $this->template->render();
  }
  ...
  public function proses($id = 0)
  {
    ...
    $this->template->write('title', 'Tambah ' . $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/form_proses', $data, true);
    $this->template->render();
  }
  ...
  public function upload_permohonan($id = '')
  {
    ...
    $this->template->write_view('content', $this->folder . '/informasi_mkg/upload_permohonan', $data, true);
    $this->template->render();
  }
  ...
  public function detail($id = 0)
  {
    ...
    $data['errMsg']     = $errMsg;
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/preview', $data, true);
    $this->template->render();
  }
  ...
  public function invoice($id = 0)
  {
    ...
    $this->template->write_view('content', $this->folder . '/informasi_mkg/invoice', $data, true);
    $this->template->render();
  }
  ...
  public function konfirmasi($id = '')
  {
    ...
    $this->template->add_js('resources/plugins/select2/select2.min.js');
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/konfirmasi', $data, true);
    $this->template->render();
  }
  ...
  public function upload($id = '')
  {
    ...
    if ($data['detail']->status == 3 || $data['detail']->status == 8) {
      ...
      if ($_POST) {
        ...
        if (!$this->upload->do_upload('bukti')) {
          ...
          $data['text_action'] = "VERIFIKASI PEMBAYARAN";
    ...
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/upload', $data, true);
    $this->template->render();
  }
  ...
  public function verifikasi_pembayaran($id = '')
  {
    ...
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/verifikasi_pembayaran', $data, true);
    $this->template->render();
  }
  ...
  public function verifikasi($id = '')
  {
    ...
    $data['detail']        = $this->db->get_where('tbl_permohonan', ['id_permohonan' => $id])->row();
    $data['detail_akun']   = $this->db->get_where('tbl_admin', ['id_admin' => $data['detail']->id_pemohon])->row();
    $data['detail_produk'] = $this->db
        // ->query("SELECT * FROM tbl_detail_permohonan WHERE id_permohonan = ? AND (status = 'Tersedia' OR status = NULL) AND penanggung_jawab = ?", [$id, $id_role])
        ->select('a.*')
        ->from('tbl_detail_permohonan a')
        ->join('m_layanan b', 'a.id_layanan = b.id_layanan')
        ->where("a.id_permohonan = " . $id)
        ->where("(a.status = 'Tersedia' OR a.status IS NULL)")
        ->where("b.penanggung_jawab = " . $id_role)
        ->get()
        ->row();

    $data['role_user']     = $this->session->userdata('id_role');
    $data['process_arr'] = get_process_array($data['detail']->id_jenis_layanan);

    if ($id_role < 8  && $id_role != 3) {
      show_404();
    }
    ...
    if ($_POST) {
      $catatan       = !empty($this->input->post('catatan', true)) ? $this->input->post('catatan') : 'Tidak ada catatan';
      $layanan       = $this->db->get_where('m_layanan', ['id_layanan' => $data['detail_produk']->id_layanan])->row();
      $ambil_di_ptsp = !empty($this->input->post('ambil_di_ptsp', true)) ? $this->input->post('ambil_di_ptsp') : null;
      $this->db->get_where('m_layanan', ['id_layanan' => $data['detail_produk']->id_layanan])->row();

      $config_1['max_size']         = 100000;
      $config_1['upload_path']      = './upload/dokumen/';
      $config_1['encrypt_name']     = true;
      $config_1['allowed_types']    = 'gif|jpg|jpeg|png|pdf|doc|docx|xls|xlsx|ppt|pptx|zip|rar';
      ...
      $total_detail = $this->db->query("SELECT count(*) as total FROM tbl_detail_permohonan WHERE id_permohonan = ?", [$id])->row()->total;
      $total_succeed_detail = $this->db->query("SELECT count(*) as total FROM tbl_detail_permohonan WHERE id_permohonan = ? AND status = 'Sukses'", [$id])->row()->total;

      if ($total_detail === $total_succeed_detail) {
          $this->db->update(
            'tbl_permohonan',
            [ 'status' => 9 ],
            [ 'id_permohonan' => $id ]
      ...
      $data['no_permohonan']  = $data['detail']->no_permohonan;
      $data['text_paragraph'] = "Sudah dilakukan konfirmasi oleh admin BO untuk";
      $data['information']    = "VERIFIKASI AKHIR";
      $data['text_action'] = "VERIFIKASI AKHIR";
      $data['link_action'] = site_url($module.'/verifikasi_akhir/'.$id);
    ...
    if (!empty($this->session->userdata('message_1'))) {
      $scsMsg = $this->session->userdata('message_1');
      $this->session->unset_userdata('message_1');
    } else if (!empty($this->session->userdata('message_2'))) {
    ...
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/verifikasi', $data, true);
    $this->template->render();
  }
  ...
  public function verifikasi_akhir($id = '')
  {
    ...
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/verifikasi_akhir', $data, true);
    $this->template->render();
  }
  ...
  public function review($id = '')
  {
    ...
    $this->template->add_js('resources/plugins/select2/select2.min.js');
    $this->template->write('title', $this->page_title);
    $this->template->write_view('content', $this->folder . '/informasi_mkg/review', $data, true);
    $this->template->render();
  }
  ...
  public function update_tahapan_proses() {
        if ($_GET) {
          redirect(site_url($this->module));
        }

        $status = 200;
        $message = "Berhasil mengubah tahapan proses";
        $is_error = false;

        $stage = (int) $this->input->post('tahapan_proses');
        $id_permohonan = (int) $this->input->post('id_permohonan');
        $id_detail_permohonan = (int) $this->input->post('id_detail_permohonan');

        $is_success = $this->db->update(
          'tbl_detail_permohonan',
          ['tahapan_proses' => $stage],
          ['id_detail_permohonan' => $id_detail_permohonan, 'id_permohonan' => $id_permohonan]
        );

        if (!$is_success) {
          $status = 500;
          $is_error = true;
          $message = "Gagal mengubah tahapan proses";
        }

        return $this->output
          ->set_content_type('application/json')
          ->set_status_header($status)
          ->set_output(json_encode(['error' => $is_error , 'message' => $message]));
  }
