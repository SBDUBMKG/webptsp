...
class Login extends CI_Controller
{
    ...
    {
        if ($this->input->post('bypass-recaptcha')) {
            return true;
        }
    ...
    private function validation()
    {
        ...
        elseif ($result && $result->status == 0) {
            $msg = "Akun anda tidak aktif atau Bukan Akun Internal PTSP";
            echo $msg;
            $this->session->set_flashdata("login_message", $msg);
            redirect(base_url() . "backend/login");
            die();
            ...
            if ($result) {
                // For chat purpose set the ac_online column to 1 when user login
                $this->db->where("id_admin", $result->id_admin)->update("tbl_admin", [ "ac_online" => 1 ]);
                ...
                // Script yang diedit Rahmat, 30 Juli 2020, penambahan kode => "OR 'admin_ptsp')"
                if ($username == "admin" or "admin_ptsp") {
                    redirect(site_url("backend/home"));
                } else {
    ...
    function logout()
    {
        // For chat purpose set the ac_online column to 0 when user logour
        $id_admin = $this->session->userdata("id_admin");
        $this->db->where("id_admin", $id_admin)->update("tbl_admin", [ "ac_online" => 0 ]);
    ...
    public function cek_login()
    {
        ...
        if ($mycaptcha != $captcha) {
            ...
            if ($username == "admin") {
                ...
                $arr_result["url_next"] = site_url(
                    "backend/edit_profil/edit_profil"
                );
            }
        } else {
            $arr_result["msg"] = "No";
