...
class Kategori_menu extends MY_Controller {
    var $page_title = 'Kategori Menu';
    var $column_datatable = array('id_kategori_menu', 'kategori_menu');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'kategori_menu_model', 'app_model');
    ...
    public function index()
    {
        ...
        $this->template->write_view('content', $this->folder.'/kategori_menu/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( $insert ) {					insert_log('Tambah kategori menu <em>'.$data_insert['kategori_menu'].'</em>');
        ...
        $this->template->write_view('content', $this->folder.'/kategori_menu/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                ...
                if ( $update ) {					insert_log('Edit kategori menu <em>'.$data_update['kategori_menu'].'</em>');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
    ...
    function delete($id = '') {		$detail = $this->app_model->get_by_id($id);		if ( $detail ) 			insert_log('Hapus kategori menu <em>'.$data_update['kategori_menu'].'</em>');
