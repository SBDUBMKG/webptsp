...
class Kabkot extends MY_Controller {
    var $page_title       	= 'Master Kabupaten/Kota';
    var $column_datatable 	= array('id_kabkot', 'kabkot', 'provinsi');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'kabkot_model', 'app_model');
    ...
    public function index() {
        ...
        $this->template->write_view('content', $this->folder.'/kabkot/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST) {
            ...
            if ( $simpan ) {
                $data_insert = array(
                    ...
                    'kabkot'        => strip_tags($this->input->post('kabkot')),
        	    );
                $insert = $this->app_model->insert_data($data_insert);

                if ( $insert ) {
                	insert_log('Tambah Master Kabkot <em>'.$data_insert['kabkot'].'</em>');
                    $this->session->set_flashdata('success', '<i class="fa fa-check"></i> Data berhasil ditambahkan');
        ...
        $this->template->write_view('content', $this->folder.'/kabkot/form', $data, true);
        $this->template->render();
    }
    ...
    function edit($id = 0) {
        ...
        if($_POST) {
            ...
            if ( $simpan ) {
                ...
                if ( $update ) {
                	insert_log('Edit Master Kabupaten/Kota <em>'.$data_update['kabkot'].'</em>');
                    $this->session->set_flashdata('success', '<i class="fa fa-check"></i> Data berhasil disimpan');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
