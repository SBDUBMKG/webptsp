...
class Satuan_berat extends MY_Controller {
    var $page_title       	= 'Master Satuan Berat Produk';
    var $column_datatable 	= array('id_satuan_berat', 'satuan_berat');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'satuan_berat_model', 'app_model');
    ...
    public function index() {
        ...
        //Baris awal penggantian tombol navigasi. Perbaikan oleh Nurhayati Rahayu (27 Mei 2024)
        $script = '
			$(function () {
				$("#datatable").DataTable({
        ...
        $this->template->write_view('content', $this->folder.'/satuan_berat/datatable', $data, true);
    ...
    function add() {
        ...
        if($_POST) {
            ...
            if ( $simpan ) {
                ...
                if ( $insert ) {
                	insert_log('Tambah Master Satuan Berat Produk <em>'.$data_insert['satuan_berat'].'</em>');
        ...
        $this->template->write_view('content', $this->folder.'/satuan_berat/form', $data, true);
        $this->template->render();
    }
    ...
    function edit($id = 0) {
        ...
        if($_POST) {
            ...
            if ( $simpan ) {
                ...
                if ( $update ) {
                	insert_log('Edit Master Satuan Berat Produk <em>'.$data_update['satuan_berat'].'</em>');
                    redirect(base_url().$module);
                } else {
                    $errMsg = 'Data gagal disimpan';
                }
