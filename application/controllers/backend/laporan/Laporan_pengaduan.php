...
class Laporan_pengaduan extends MY_Controller {
    var $page_title = 'Laporan Pengaduan';
    var $column_datatable = array('id_pengaduan', 'waktu_pengaduan','nama','email','pengaduan','response');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'laporan_pengaduan_model', 'app_model');
    ...
    public function index()
    {
                    ...
                    buttons: [
                      ...
                      },
                      {
                        ...
                        customize : function(doc) {
                           doc.content[1].table.widths = ["5%", "10%", "15%", "auto", "*", "*"];
        ...
        $this->template->write_view('content', $this->folder.'/laporan_pengaduan/datatable', $data, true);
    ...
    private function valid_form($act = 'add') {
        ...
        $config = array(
                    array(
                        'field' => 'waktu_pengaduan',
                        'label' => 'waktu_pengaduan',
                        'rules' => 'required'
                    ),
    ...
    function add() {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
                'waktu_pengaduan' => $this->input->post('waktu_pengaduan'),
                'nama' => strip_tags($this->input->post('nama')),
                'email' => strip_tags($this->input->post('email')),
                'pengaduan' => $this->input->post('pengaduan'),
                'response' => strip_tags($this->input->post('response')),
            );
        ...
        $this->template->write_view('content', $this->folder.'/laporan_pengaduan/form', $data, true);
        $this->template->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
