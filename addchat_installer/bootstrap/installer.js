		...
		$.ajax({
			...
			success: function(result) {
	        	if(result.response) {
					...
					$('.text-success.d_config_text').html('Congrats! Your website successfully configured for addChat!');
		...
		$.ajax({
			...
			success: function(result) {
	        	if(result.response) {
					...
					$('.text-success.d_transfer_text').html('Congrats! addChat files transferred successfully!');
		...
		$.ajax({
			...
			success: function(result) {
	        	if(result.response) {
					...
					$('.text-success.d_database_text').html('Congrats! addChat Tables imported successfully!');
...
$(function() {
	...
	$('#d_finish_btn').on('click', function() {
		if( ($('input[name="dbcharset"]:checked').length > 0) && ($('input[name="ac_init"]:checked').length > 0) ) {
			var w_url = base_url.replace('addchat_installer/', '');
